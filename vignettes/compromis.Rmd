...
#import data
data <- as.matrix(read.csv('../data/big_simulation3.csv', header = T, sep = ';', dec = ',')[,-26])
...
# constraint <- list(seuil_EI_QC = CDC$seuil[which(! is.na(CDC$seuil))],
#                    index_seuil = which(! is.na(CDC$seuil)))
...
X_scale <- scale(data)
X_decision <- pareto_finding (X = X_scale, Y = res, method = 'target', target = c(2,3), n = 5, plot = TRUE)
```
...
for (i in 1:nrow(X_decision$X)){
  ...
  class(cow) <- 'cow'
  plot(cow, choice = 'radar_diag',  radial.lim = c(-3,3))
  
  dev.copy(png, paste('../plot/vache', i, '.png'))
  dev.off()
